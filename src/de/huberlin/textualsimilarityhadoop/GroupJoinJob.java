...
public class GroupJoinJob extends Configured implements Tool {
  private static final Logger log = Logger.getLogger(GroupJoinJob.class);
...
//    private final GroupJoinVal outVal = new GroupJoinVal(); // <<< nur für Statistik
...
//              GroupJoinKey[] previousProbeKeys = currentVal1.getIndexKeysFor(delta, l);// INDEX KEY!!!
   ...
   */
  public static class GroupJoinCountReducer extends Reducer<GroupJoinKey, GroupJoinVal, Text, NullWritable> {
    Text outKey = new Text();
    NullWritable outVal = NullWritable.get();
    ...
    InterruptedException {
      ...
      maxNumberOfRecords = Integer.MIN_VALUE;
      minNumberOfRecords = Integer.MAX_VALUE;
    }
    
    @Override
    protected void cleanup(Context context) throws IOException,
      InterruptedException {
      outKey.set(numberOfPartitions + " " + totalNumberOfRecords + " " + maxNumberOfRecords + " " + minNumberOfRecords);
    ...
    public void reduce(GroupJoinKey key, Iterable<GroupJoinVal> values, Context context) throws IOException, InterruptedException {
      ...
      if (maxNumberOfRecords < count) {
        maxNumberOfRecords = count;
      }
      if (minNumberOfRecords > count) {
        minNumberOfRecords = count;
      }
...
//    log.setLevel(Level.ALL);
    ...
    job1a.setJarByClass(GroupJoinJob.class);
    ...
    if (onlyStatistics) {
      job1a.setReducerClass(GroupJoinCountReducer.class);
    } else {
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new Configuration(), new GroupJoinJob(), args);
  }

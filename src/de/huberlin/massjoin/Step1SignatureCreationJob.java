 ...
 * @author fabi
 */
public class Step1SignatureCreationJob extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    job1.setJarByClass(Step1SignatureCreationJob.class);
    
    job1.setMapperClass(SignatureMapper.class);
    ...
    if (onlyStatistics) {
      job1.setReducerClass(CandidateCreationStatisticsReducer.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new Configuration(), new Step1SignatureCreationJob(), args);
  }
  
  public static class SignatureMapper extends Mapper<Object, Text, MassJoinSignatureKey, MassJoinIntermediateElem> {
...
//      ArrayList<MassJoinIntermediateElem> s = new ArrayList();
      ...
      while (it.hasNext()) {
        ...
        if (current.getPartition() == 0) {
          try {
            ...
            Logger.getLogger(Step1SignatureCreationJob.class.getName()).log(Level.SEVERE, null, ex);
          }
        } else {
  ...
  public static class CandidateCreationStatisticsReducer extends Reducer<MassJoinSignatureKey, MassJoinIntermediateElem, IntWritable, Text> {
    private final IntWritable outKey = new IntWritable();
    private final Text outValue = new Text();
    ...
    public void reduce(MassJoinSignatureKey key, Iterable<MassJoinIntermediateElem> values, Context context) throws IOException, InterruptedException {
      ...
      while (it.hasNext()) {
        MassJoinIntermediateElem current = it.next();
        if (current.getPartition() == 0) { // r:
          rCount++;
        } else { // s
          sCount++;
        }
      }
      outValue.set(rCount + " " + sCount);
      outKey.set(context.getTaskAttemptID().getTaskID().getId());
      context.write(outKey, outValue);
    }
  }
}

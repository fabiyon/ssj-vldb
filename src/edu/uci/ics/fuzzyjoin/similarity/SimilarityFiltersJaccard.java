...
public class SimilarityFiltersJaccard implements SimilarityFilters {
    ...
    public SimilarityFiltersJaccard(float similarityThreshold, int fixedL) {
    ...
    public int getPrefixLength(int length) {
        ...
        return length - (int) Math.ceil(simThr * length) + fixedL;
    }
    ...
    public float passSimilarityFilter(final int[] tokensX, int startX,
        ...
        // this code contains errors or relies on properties of the data we don't know of:
//        if (token < tokenProbe) {
...
//            }
//        }
        //  we replace it by a naive implementation:
        for (int i = 0; i < tokensX.length; i++) {
          for (int j = 0; j < tokensY.length; j++) {
            if (tokensY[j] == tokensX[i]) {
              intersectSize++;
            }
          }
        }
        ...
        if (intersectSize >= intersectSizeLowerBound) {
          float similarity = ((float) intersectSize) / (length + lengthProbe - intersectSize);
          if (similarity > simThr) {
            return similarity;
          }
        }
        return 0;
